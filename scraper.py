...
URL = "https://fbref.com/en/comps/13/2020-2021/2020-2021-Ligue-1-Stats"
page = requests.get(URL)
soup = BeautifulSoup(page.content, "html.parser")
table = soup.select_one('#results2020-2021131_overall>tbody')
        ...
        if team_index > 0:
            if game_index <= 10000:
                ...
                for match_row in league_matches:
                    if game_index <= 123:
                        match_page = requests.get(
                            f'{BASE_URL}{match_row.select_one("td[data-stat=match_report]>a")["href"]}')
                        match_page_content = BeautifulSoup(match_page.content, "html.parser")

                        print(f'{BASE_URL}{match_row.select_one("td[data-stat=match_report]>a")["href"]}')

                        # Dane meczu
                        match_date_string = \
                            match_page_content.select_one('.scorebox .scorebox_meta div:first-of-type strong a')[
                                'href'].split(
                                '/')[
                                -1]
                        match_date_array = match_date_string.split('-')
                        match_date = datetime.date(int(match_date_array[0]), int(match_date_array[1]),
                                                   int(match_date_array[2]))
                        ...
                        print('match data scraped')

                        # Dane gospodarza
                        home_team_score = get_digits(
                            match_page_content.select_one('.scorebox>div:first-of-type>.scores>.score').text)
                        home_team_xG = match_page_content.select_one('.scorebox div:first-of-type .scores .score_xg')
                        ...
                        home_team_coach = \
                            match_page_content.select_one('.scorebox>div:first-of-type>div:nth-of-type(5)').text.split(
                                ':')[1]
                        ...
                        home_team_red_cards = len(match_page_content.select(
                            'tr > td:nth-of-type(1) .red_card, tr > td:nth-of-type(1) .yellow_red_card'))

                        print('home team data scraped')

                        # Dane gościa
                        away_team_score = get_digits(
                            match_page_content.select_one('.scorebox>div:nth-of-type(2)>.scores>.score').text)
                        away_team_xG = match_page_content.select_one('.scorebox div:nth-of-type(2) .scores .score_xg')
                        ...
                        away_team_coach = \
                            match_page_content.select_one('.scorebox>div:nth-of-type(2)>div:nth-of-type(5)').text.split(
                                ':')[1]
                        ...
                        print('away team data scraped')

                        # Dane zawodnikow
                        index_getter = operator.itemgetter(0, 2)
                        players_stats_tables = index_getter(
                            match_page_content.findAll('div', {'class': 'table_wrapper'}))
                        ...
                        for table in players_stats_tables:
                            ...
                            players_passes_completed = table.select('td[data-stat=passes_coempleted]:not(tfoot *)')
                            ...
                            players_names = list(filter(lambda x: x != '-',
                                                        [a.text if a.text[2:] != ' Players' else '-' for a in
                                                         players_names]))
                            ...
                            if len(home_players):
                                print('players data scraping... 2/2')
                                away_players = team_players
                            else:
                                print('players data scraping... 1/2')
                        ...
                        home_coach_full_name = unicodedata.normalize('NFKD', home_team_coach).encode('ascii',
                                                                                                     'ignore').decode(
                            'utf-8').strip()
                        ...
                        away_coach_full_name = unicodedata.normalize('NFKD', away_team_coach).encode('ascii',
                                                                                                     'ignore').decode(
                            'utf-8').strip()
                        ...
                        add_home_team = insert(Team).returning(Team.id).values(
                            name=unicodedata.normalize('NFKD', home_team).encode('ascii', 'ignore').decode(
                                'utf-8').strip(), season=season, league=league_index)
                        add_home_team = add_home_team.on_conflict_do_update(
                            constraint='name_and_season_unique',
                            set_=dict(league=league_index)
                        )

                        add_away_team = insert(Team).returning(Team.id).values(
                            name=unicodedata.normalize('NFKD', away_team).encode('ascii', 'ignore').decode(
                                'utf-8').strip(), season=season, league=league_index)
                        ...
                        for player in home_players + away_players:
                            add_player = insert(PlayerTable).returning(PlayerTable.id).values(
                                full_name=unicodedata.normalize('NFKD', player.name).encode('ascii', 'ignore').decode(
                                    'utf-8').strip(), birthday=player.age,
                                nationality=player.country).on_conflict_do_update(
